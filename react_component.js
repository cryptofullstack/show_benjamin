        ...
        <div className="main-board">
            <div className="main-board-title-div d-flex">
                <div className="main-board-title-current-value-div">
                    {getTotalVal.totalValueStr.length < 11 ? (<span>Current Value</span>) : <></>}
                ...
                <Box sx={{ minWidth: 120 }}>
                    <FormControl fullWidth>
                        ...
                        >
                            ...
                            <MenuItem onClick={() => onFilterChain('matic')} value={'Polygon'}>Polygon</MenuItem>
                            <MenuItem onClick={() => onFilterChain('avax')} value={'Avalanche'}>Avalanche</MenuItem>
                            <MenuItem onClick={() => onFilterChain('ftm')} value={'Fantom'}>Fantom</MenuItem>
                        </Select>
                    </FormControl>
                </Box>
            </div>
            <div className="table-board">
                    ...
                    <tbody>
                        {portfolio_table_body_data.map((item, index) => {
                            return <><tr key={index} className="accordion-toggle table-tr"
                                id="accordion1" >
                                {item.history ? item.history.length > 0 ? <td className="table-body expand-button td_symbol collapsed" data-toggle="collapse" href={`#${item.id + item.chain}`} ></td> :
                                    <td></td> : <td></td>}
                                <td className="table-body vertical-align-middle"><img className="table-td-s-image" src={item.type_img} />{item.type}</td>
                ...
                <FootNote portfolioDataTx={portfolioPreData} />
            </div>
        </div>
    )
}
